...
export const useTimezones = () => {
    ...
    const swapFirstLast = () => {
        if (timezones.length > 1) {
            const swappedTimezones = [...timezones];
            const temp = swappedTimezones[0];
            swappedTimezones[0] = swappedTimezones[swappedTimezones.length - 1];
            swappedTimezones[swappedTimezones.length - 1] = temp;
            setTimezones(swappedTimezones);
        }
    };

    return { timezones, addTimezone, removeTimezone, reorderTimezones, swapFirstLast };
};
